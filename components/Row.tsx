...
function Row({ title, videos }: Props) {
  // Default to the favorited video, or index 0
  const favoriteIndex = videos.findIndex((v) => v.isFavorite);
  ...
  return (
    ...
    >
      <h2 className="text-xl md:text-3xl font-semibold text-[#85caff] mb-8 md:mb-12 tracking-wide uppercase">
        {title}
      </h2>

      {/* Carousel Container — takes up the main area */}
      <div className="relative w-full flex-1 max-h-[65vh] flex items-center justify-center">
        ...
        <div className="w-full h-full relative" style={{ perspective: '1200px' }}>
          {videos.map((video, index) => {
            ...
            // Center card: large. Side cards: smaller, pushed out, dimmed
            const scale = isCenter ? 1.0 : 0.7;
            const translateX = isCenter ? 0 : offset * 38; // percentage-based
            ...
            return (
              ...
              >
                <Thumbnail video={video} isActive={isCenter} />
              </div>
            );
          })}
        </div>
